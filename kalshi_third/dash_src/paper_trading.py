...
# ── REFIT model: logistic P(win) on [|delta_from_open|, delta_from_open, tau_seconds].
#    Baked from PROD ticks/signals 2026-05-25..06-22 (walk-forward OOS fill-robust +$3.96/trade).
#    Gate (s_refit): trade only when P(win) > market entry price. Features standardized. ──
...
def _refit_pfit(dfo, tau_min):
    import math
    try:
        tau_s = float(tau_min or 0) * 60.0          # live tau is MINUTES; model trained on SECONDS
        feats = [abs(float(dfo)), float(dfo), tau_s]
        z = _REFIT["intercept"] + sum(c * (f - m) / s for c, f, m, s in zip(_REFIT["coef"], feats, _REFIT["mu"], _REFIT["sd"]))
...
DEFAULT_CONFIG = {
    ...
    "ofi_align": "off",
    "ev_thr": 0.0,
    # REFIT_AB — logistic P(win)>entry gate (s_refit). "" = off (legacy), "1" = on.
    "refit_gate": "",
    "refit_margin": 0.0,
}
...
def update_session_config(sid, config_updates):
    ...
    with paper_lock:
        ...
        for k, v in config_updates.items():
            if k in s["config"]:
                # Type conversion
                if k in ("kappa", "delta_threshold", "p_model_threshold", "max_entry_price", "stake", "min_stake", "threshold_gap", "entry_wait_min", "vol_imb_kill", "ev_thr"):
                    s["config"][k] = float(v)
                elif k == "paused":
                    s["config"][k] = bool(v)
                else:
                ...
                if k.startswith("cfg_"):
                    ...
                    if real_key in cfg:
                        if real_key in ("kappa", "delta_threshold", "p_model_threshold",
                                        "max_entry_price", "stake", "min_stake", "threshold_gap", "entry_wait_min", "vol_imb_kill", "ev_thr"):
...
def _evaluate_signal(session_cfg, shared):
    ...
    # Sigma cap (2026-06-17 re-opt: cut top vol tercile → walk-forward +$1.47→+$3.12/trade).
    _sigmax = session_cfg.get("sigma_max")
    if _sigmax and sigma and sigma > float(_sigmax):
        return f"SIGMA-HIGH ({sigma:.2e}>{float(_sigmax):.2e})", None, None, None
    ...
    # ── REFIT gate (s_refit A/B): only trade when refit P(win) > market entry ──
    if session_cfg.get("refit_gate"):
        _pf = _refit_pfit(delta, tau)
        if _pf is None or _pf <= entry_price + float(session_cfg.get("refit_margin", 0.0) or 0.0):
            return (f"REFIT-SKIP ({_pf:.2f}<=e{entry_price:.2f})" if _pf is not None else "REFIT-ERR"), None, None, None
...
def _session_liq(cfg, side, shared):
    ...
    if str(cfg.get("liq_filter", "on")) == "on":
